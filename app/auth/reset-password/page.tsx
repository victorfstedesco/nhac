...
export default function ResetPasswordPage({ searchParams }: { searchParams: Promise<{ token?: string }> }) {
  ...
  if (!token) {
    return (
      <div className="flex flex-col gap-6 w-full max-w-sm mx-auto sm:mx-0">
        ...
        <Link href="/auth/forgot-password" className="text-sm font-bold text-brand hover:text-brand-dark transition-colors underline underline-offset-4">
          Solicitar link
        </Link>
      </div>
    );
    ...
    return (
      <div className="flex flex-col gap-6 w-full max-w-sm mx-auto sm:mx-0">
        <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-extrabold text-zinc-900">Senha redefinida!</h1>
          <p className="text-base text-zinc-500">Redirecionando para o login...</p>
        </div>
      </div>
    );
  }
